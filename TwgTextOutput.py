...
APP_TITLE = "トワイライトガンスモーク キャラクターシート抽出"
...
def extract_page_data(driver):
    """
    input / textarea / select の現在値を取得し、
    見出しや表と一緒に抽出する。
    """
    ...
    function getCellText(cell) {
        ...
        cloneControls.forEach(
            (cloneControl, index) => {
                const original =
                    originalControls[index];
...
class App(tk.Tk):
    ...
    def run_export(self):
        ...
        # トワイライトガンスモークのURLか確認
        if (
            "character-sheets.appspot.com/tgs/"
            not in url
        ):
        ...
        output_path = filedialog.asksaveasfilename(
            ...
            initialfile=(
                "twilight_gunsmoke_character.txt"
            )
        )
